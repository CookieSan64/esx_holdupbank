...
local ServerConfig = {
    webhooks = "https://discord.com/api/webhooks/1090282324266057770/MXd3Eqs6VNuxIeHJA1bRMEu75u4p0_TyvpnrGOKimlwcntZNFxrdnHGA4OrUYch3iUXY",
	...
	if Stores[currentStore] then
		...
		if not rob then
			if cops >= Config.PoliceNumberRequired then
				...
				ServerToDiscord(ServerConfig.webhooksTitle, '[Braquage Lancé] __'  ..xPlayer.getName().. '__ vient de lancer un braquage de Banque (`'..store.nameOfStore..'`)', ServerConfig.webhooksColor)

				SetTimeout(2000, function()
					while robbers[_source] and timeElapsed < store.secondsRemaining do
						xPlayer.addInventoryItem('moneycase', 2)
					...
					if robbers[_source] then
						...
						if xPlayer then
							...
							SetTimeout(2000, function()
								while robbers[_source] and timeElapsed < store.secondsRemaining do
									xPlayer.addInventoryItem('moneycase', 2)
									nbMoneyCases = nbMoneyCases + 2
									timeElapsed = timeElapsed + 2
									Citizen.Wait(2000)
								end
								TriggerClientEvent('esx_holdupbank:robberyComplete', _source, 600)
								if Config.GiveBlackMoney then
									ServerToDiscord(ServerConfig.webhooksTitle, '[Braquage Réussi] __' ..xPlayer.getName().. '__ vient de réussir son braquage de la Banque : (`'..store.nameOfStore..'`) il a obtenu 600 sac(s) d\'argent', ServerConfig.webhooksColor2)
								end
							end)
